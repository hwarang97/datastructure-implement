...
void alternateMergeLinkedList(LinkedList *ll1, LinkedList *ll2)
{
	...
	int iter_time = ll1->size;
	int i = 0;

	// insert ll2 item into ll1 alternativly
	while (i < iter_time && ll2->head != NULL)
	{
		ListNode *node_pointer_to_insert = ll2->head;
		if (insertNode(ll1, 2 * i + 1, node_pointer_to_insert->item) != -1)
		{
			removeNode(ll2, 0);
		}
		i++;
